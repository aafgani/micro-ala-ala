# how to run:
#   bash infra/tools/image/push-web-client.sh

# This script builds and pushes a Docker image for the Web Client application to Azure Container Registry (ACR).
...
# and that the Dockerfile is located at `src/apps/App.Web.Client/dockerfile`.
...
# === [3] Build & Push ===
docker build -t "$ACR_IMAGE" -f src/apps/App.Web.Client/dockerfile . --no-cache
